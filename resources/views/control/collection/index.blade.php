...
@section('title') Коллекции @endsection
    ...
    <section class="w-full flex justify-center mt-8">
        <div class="container">
            ...
            <nav class="flex mb-6" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            <svg class="w-6 h-6 text-gray-600" fill="currentColor" stroke-width="1" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                            <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2">Коллекции</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <div class="w-full flex justify-between items-center mb-8">
                <h2 class="text-xl text-gray-900">Коллекции</h2>
            </div>
            ...
            <div class="w-full flex items-start">
                <ul class="w-1/2 text-sm font-medium text-gray-900 bg-white border border-gray-100 rounded-md">
                    @foreach($collections as $collection)
                        <li class="w-full flex justify-between items-center px-4 py-2 border-b border-gray-200 rounded-t-lg">
                            @if(session()->has("edit-{$collection->id}"))
                                <form action="{{ route('collection.update', ['collection' => $collection->id]) }}" method="POST" class="w-full flex items-center mr-10">
                                    @csrf
                                    @method('PATCH')
                                    <div class="w-full">
                                        <input type="text" name="title-update" id="title" class="@error('title') bg-red-50 border border-red-500 text-red-900 placeholder-red-700 @enderror
                                            bg-gray-50 border border-gray-100 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2"
                                               placeholder="Нименование" value="{{ $collection->title }}">
                                        @error('title')
                            ...
                            @endif
                            <div class="flex items-center">
                                <a href="{{ route('collection.edit', ['collection' => $collection->id]) }}">
                                ...
                                <form action="{{ route('collection.destroy', ['collection' => $collection->id]) }}" method="POST" onsubmit="confirmation(event)">
                                    @csrf
                                    @method('DELETE')
                ...
                <form action="{{ route('collection.store') }}" method="POST" class="w-1/2 ml-12 flex flex-col items-end">
                    @csrf
                    <div class="w-full">
                        <label for="title" class="block mb-2 text-base text-gray-900">Наименование города</label>
