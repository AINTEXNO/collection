...
@section('content')
    ...
    <section class="w-full flex justify-center mt-8">
        <div class="container">
            ...
            <nav class="flex mb-6" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            <svg class="w-6 h-6 text-gray-600" fill="currentColor" stroke-width="1" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                            <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2">Магазины</span>
                        </div>
                    </li>
                </ol>
            </nav>
                    ...
                    @foreach($shops as $shop)
                        <tr class="bg-white border-b border-gray-100">
                            ...
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 flex items-center">
                                <a href="{{ route('shops.edit', ['shop' => $shop->id]) }}" class="font-medium text-indigo-600 hover:underline">Редактировать</a>
                                <form action="{{ route('shops.destroy', ['shop' => $shop->id]) }}" method="POST" class="ml-4" onsubmit="confirmation(event)">
                                    @csrf
                                    @method('DELETE')
            ...
            <a href="{{ route('shops.create') }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Добавить магазин</a>
        </div>
    </section>
