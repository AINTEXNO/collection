...
@section('content')
            ...
            <nav class="flex mb-4" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            <svg class="w-6 h-6 text-gray-600" fill="currentColor" stroke-width="1" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                            <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2">Редактировать товар</span>
                        </div>
                    </li>
                </ol>
            </nav>
    ...
    <section class="w-full h-[calc(100vh-200px)] flex justify-center items-center">
        <div class="container">
            <form action="{{ route('product.update', ['product' => $product->id]) }}" method="POST" class="w-full flex flex-col items-start" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                    ...
                    <div class="w-1/2 border border-gray-150 rounded-sm box-border py-5 px-6">
                        ...
                        <div class="w-full flex flex-col items-start">
                            ...
                            <div class="w-full flex mb-3">
                                ...
                                <div class="w-full flex flex-col items-start">
                                    <input type="text" name="title" id="title" class="@error('title') border-red-400 @enderror w-full border py-1.5 px-3 mt-1 text-base rounded-sm" placeholder="Наименование товара" value="{{ $product->title }}">
                            ...
                            <div class="w-full flex mb-3">
                                ...
                                <div class="w-full flex flex-col items-start">
                                    <textarea name="description" id="description" class="@error('description') border-red-400 @enderror resize-none w-full h-20 border py-1.5 px-3 mt-1 text-base rounded-sm" placeholder="Описание товара">{{ $product->description }}</textarea>
                            ...
                            <div class="w-full flex mb-3">
                                ...
                                <div class="w-full flex flex-col items-start">
                                    <input type="number" name="price" id="price" min="1" step="0.01" class="@error('price') border-red-400 @enderror w-1/2 border py-1.5 px-3 mt-1 text-base rounded-sm" placeholder="Стоимость" value="{{ $product->price }}">
                            ...
                            <div class="w-full flex mb-3">
                                ...
                                <div class="w-full flex flex-col items-start">
                                    <input type="number" name="count" id="count" min="1" class="@error('count') border-red-400 @enderror w-1/2 border py-1.5 px-3 mt-1 text-base rounded-sm" placeholder="Количество" value="{{ $product->count }}">
                    ...
                    <div class="w-1/2 border border-gray-150 rounded-sm box-border py-5 px-6 ml-6">
                        ...
                        <div class="w-full flex flex-col items-start">
                                ...
                                <div class="w-full flex flex-col items-start">
                                    <select name="collection_id" id="collection_id" class="@error('collection_id') border-red-400 @enderror w-full border py-1.5 px-3 mt-1 text-base rounded-sm">
                                        ...
                                        @foreach($collections as $collection)
                                            <option value="{{ $collection->id }}" @if($collection->id === $product->collection->id) selected @endif>{{ $collection->title }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="w-full flex flex-col items-start">
                                    <select name="style_id" id="style_id" class="@error('style_id') border-red-400 @enderror w-full border py-1.5 px-3 mt-1 text-base rounded-sm">
                                        ...
                                        @foreach($styles as $style)
                                            <option value="{{ $style->id }}" @if($style->id === $product->style->id) selected @endif>{{ $style->title }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="w-full flex flex-col items-start">
                                    <select name="brand_id" id="brand_id" class="@error('brand_id') border-red-400 @enderror w-full border py-1.5 px-3 mt-1 text-base rounded-sm">
                                        ...
                                        @foreach($brands as $brand)
                                             <option value="{{ $brand->id }}" @if($brand->id === $product->brand->id) selected @endif>{{ $brand->title }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="w-full flex flex-col items-start">
                                    <select name="color_id" id="color_id" class="@error('color_id') border-red-400 @enderror w-full border py-1.5 px-3 mt-1 text-base rounded-sm">
                                        ...
                                        @foreach($colors as $color)
                                            <option value="{{ $color->id }}" @if($color->id === $product->color->id) selected @endif>{{ $color->title }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="w-full flex justify-end">
                                <button type="submit" class="px-6 py-2.5 text-sm bg-gray-200 text-gray-600 rounded-full uppercase text-black mt-7">Изменить товар</button>
                            </div>
                        </div>
                    </div>
                </div>
