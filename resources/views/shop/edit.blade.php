...
@section('title') Редактирование магазина @endsection
...
@section('content')
            ...
            <nav class="flex mb-6" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            <svg class="w-6 h-6 text-gray-600" fill="currentColor" stroke-width="1" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                            <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2">Редактировать магазин</span>
                        </div>
                    </li>
                </ol>
            </nav>
    ...
    <section class="w-full h-[calc(100vh-200px)] flex justify-center items-center">
        <div class="container">
            <form action="{{ route('shops.update', ['shop' => $shop->id]) }}" method="POST" class="w-full flex flex-col items-start" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                    ...
                    <div class="w-1/2 border border-gray-150 rounded-sm box-border py-5 px-6">
                        @if(session()->has('updated'))
                            <div class="w-full p-4 mb-4 text-sm text-blue-700 font-normal bg-blue-100 rounded-lg" role="alert">
                                <span class="font-normal">Данные магазина успешно изменены</span>
                            </div>
                        @endif
                        ...
                        <div class="w-full flex flex-col items-start">
                            <div class="w-full flex mb-3">
                                ...
                                <div class="w-full flex flex-col items-start">
                                    <input type="text" name="title" id="title" class="@error('title') border-red-400 @enderror w-full border py-1.5 px-3 mt-1 text-base rounded-sm" placeholder="Наименование магазина" value="{{ $shop->title }}">
                            ...
                            <div class="w-full flex mb-3">
                                ...
                                <div class="w-full flex flex-col items-start">
                                    <input type="text" name="address" id="address" class="@error('address') border-red-400 @enderror w-full border py-1.5 px-3 mt-1 text-base rounded-sm" placeholder="Адрес магазина" value="{{ $shop->address }}">
                            ...
                            <div class="w-full flex mb-3">
                                ...
                                <div class="w-full flex flex-col items-start">
                                    <input type="text" name="schedule" id="schedule" class="@error('schedule') border-red-400 @enderror w-full border py-1.5 px-3 mt-1 text-base rounded-sm" placeholder="Адрес магазина" value="{{ $shop->schedule }}">
                            ...
                            <div class="w-full flex mb-3">
                                ...
                                <div class="w-full flex flex-col items-start">
                                    <input type="text" name="phone_number" id="phone_number" class="@error('phone_number') border-red-400 @enderror w-full border py-1.5 px-3 mt-1 text-base rounded-sm" placeholder="Номер телефона" value="{{ $shop->getRawOriginal('phone_number') }}">
                                    @error('phone_number')<p class="text-base text-red-400 my-2">{{ $message }}</p>@enderror
                                </div>
                            </div>
                            <div class="w-full flex justify-end">
                                <button type="submit" class="px-6 py-2.5 text-sm bg-gray-200 text-gray-600 rounded-full uppercase text-black mt-7">Изменить</button>
                            </div>
                        </div>
                    </div>
                </div>
